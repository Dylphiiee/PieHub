...
local quickAnimPanelEnabled = false
local _quickAnimGuiRef = nil
local _quickAnimPanelRef = nil
...
SectionAnim:Toggle({
    ...
    Desc = "Aktifkan tombol di pojok kiri bawah untuk akses cepat animations & emotes",
    Icon = "layout",
    Value = false,
    Callback = function(state)
        quickAnimPanelEnabled = state
        if _quickAnimGuiRef then _quickAnimGuiRef.Enabled = state end
        if not state and _quickAnimPanelRef then _quickAnimPanelRef.Visible = false end
    end
})
...
-- Wire forward refs setelah panel dibuat
_quickAnimGuiRef = QuickAnimGui
_quickAnimPanelRef = QuickAnimPanel
