...
-- ==================== JUMP FIX ====================
local function repairHumanoid()
    local hum = getHum()
    if not hum then return end
    -- pastikan jump power tidak 0
    pcall(function()
        if hum.JumpPower == 0 then
            hum.JumpPower = 50
        end
        -- pastikan state jumping diaktifkan
        hum:SetStateEnabled(Enum.HumanoidStateType.Jumping, true)
        hum.PlatformStand = false
        -- ganti ke state normal agar bisa lompat
...
local function applyAnimation(data)
    ...
    repairHumanoid() -- fix jump after animation pack
end
...
local function resetAnimation()
    ...
    Animate.Disabled = true task.wait(0.05) Animate.Disabled = false
    defaultAnimIds = {}
    repairHumanoid() -- fix jump after reset
end
...
local function stopEmote()
    ...
    repairHumanoid() -- fix jump after stopping emote
end
...
local quickPanelEnabled = false
local quickPanelGui = nil
local quickPanelMode = "Animations" -- "Animations" or "Emotes"
local quickSearchText = ""

local function buildQuickAnimList()
    local filtered = {}
    local q = quickSearchText:lower()
    for _, v in ipairs(AnimationList) do
        if q == "" or v.Title:lower():find(q, 1, true) then
            table.insert(filtered, v.Title)
        end
    end
...
local function buildQuickEmoteList()
    local filtered = {}
    local q = quickSearchText:lower()
    for _, v in ipairs(FullEmoteList) do
        if q == "" or v.Title:lower():find(q, 1, true) then
...
local function refreshQuickPanel()
    if not quickPanelGui then return end
    local frame = quickPanelGui:WaitForChild("Frame", 1)
    if not frame then return end
    local scroll = frame:FindFirstChild("Scroll")
    if not scroll then return end
    local listFrame = scroll:WaitForChild("List", 1)
    ...
    local items
    if quickPanelMode == "Animations" then
        items = buildQuickAnimList()
    else
        items = buildQuickEmoteList()
    end
    ...
    for _, name in ipairs(items) do
        ...
        btn.MouseButton1Click:Connect(function()
            if quickPanelMode == "Animations" then
                local data = getAnimData(name)
                if data then
                    applyAnimation(data)
                    WindUI:Notify({ Title = "Quick Panel", Content = "Animasi diterapkan: " .. name, Duration = 1.5, Icon = "play" })
                end
            else
                local data = getEmoteData(name)
                if data then
                    playEmote(data, emoteLoopEnabled, emoteWalkEnabled)
                    WindUI:Notify({ Title = "Quick Panel", Content = "Emote dimainkan: " .. name, Duration = 1.5, Icon = "play" })
                end
            end
        end)
...
local function createQuickPanel()
    if quickPanelGui then quickPanelGui:Destroy() end
    quickPanelGui = Instance.new("ScreenGui")
    quickPanelGui.Name = "PieHubQuickPanel"
    quickPanelGui.ResetOnSpawn = false
    pcall(function() quickPanelGui.Parent = CoreGui end)
    if not quickPanelGui.Parent then quickPanelGui.Parent = LocalPlayer:WaitForChild("PlayerGui") end

    local frame = Instance.new("Frame", quickPanelGui)
    frame.Name = "Frame"
    ...
    searchBox.Text = quickSearchText
    searchBox.Font = Enum.Font.Gotham
    searchBox.TextSize = 13
    searchBox.ClearTextOnFocus = false

    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        quickSearchText = searchBox.Text
        refreshQuickPanel()
    end)

    local btnAnim = Instance.new("TextButton", frame)
    btnAnim.Name = "BtnAnim"
    btnAnim.Size = UDim2.new(0.5, -7, 0, 25)
    btnAnim.Position = UDim2.new(0, 5, 0, 60)
    btnAnim.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    btnAnim.Text = "Animations"
    btnAnim.TextColor3 = Color3.fromRGB(255, 255, 255)
    btnAnim.TextSize = 13
    btnAnim.Font = Enum.Font.GothamSemibold
    local btnEmote = Instance.new("TextButton", frame)
    btnEmote.Name = "BtnEmote"
    btnEmote.Size = UDim2.new(0.5, -7, 0, 25)
    btnEmote.Position = UDim2.new(0.5, 2, 0, 60)
    btnEmote.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    btnEmote.Text = "Emotes"
    btnEmote.TextColor3 = Color3.fromRGB(255, 255, 255)
    btnEmote.TextSize = 13
    btnEmote.Font = Enum.Font.GothamSemibold

    local function updateModeButtons()
        btnAnim.BackgroundColor3 = quickPanelMode == "Animations" and Color3.fromRGB(80, 80, 80) or Color3.fromRGB(45, 45, 45)
        btnEmote.BackgroundColor3 = quickPanelMode == "Emotes" and Color3.fromRGB(80, 80, 80) or Color3.fromRGB(45, 45, 45)
    end
    btnAnim.MouseButton1Click:Connect(function()
        quickPanelMode = "Animations"
        updateModeButtons()
        refreshQuickPanel()
    end)
    btnEmote.MouseButton1Click:Connect(function()
        quickPanelMode = "Emotes"
        updateModeButtons()
        refreshQuickPanel()
    end)
    updateModeButtons()
...
local SectionQuickPanel = TabAnimations:Section({ Title = "Quick Panel", Icon = "layout-panel-top", Opened = true })
SectionQuickPanel:Toggle({
    Title = "Enable Floating Panel", Desc = "Tampilkan panel kecil untuk akses cepat Animations & Emotes",
    Icon = "layout-panel-top", Value = false,
    Callback = function(state)
        quickPanelEnabled = state
        if state then
            createQuickPanel()
        else
...
SectionVisualMore:Toggle({
    Title = "Freecam", Desc = "Detach camera and fly around freely", Icon = "video", Value = false,
    Callback = function(state)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    repairHumanoid() -- pastikan jump normal setelah respawn
end)
