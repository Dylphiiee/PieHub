...
local targetDropdown = SectionTarget:Dropdown({
    Title = "Target List", Desc = "Pilih pemain yang akan jadi target",
    Values = getPlayerList(), Value = "",
    Callback = function(value)
        for _, v in pairs(Players:GetPlayers()) do
            if v.Name == value then targetPlayer = v break end
        end
    end
})
...
SusSection:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            currentPlatform.Transparency = 1 currentPlatform.Anchored = true
            currentPlatform.CanCollide = true currentPlatform.Parent = workspace
            activeLoops["doggy"] = RunService.Heartbeat:Connect(function()
                if activeStates["doggy"] and targetPlayer and isAlive(targetPlayer) and getPlayerHRP(targetPlayer) and getPlayerHRP(LocalPlayer) then
                    ...
                    local frontPos = targetPos + getPlayerHRP(targetPlayer).CFrame.LookVector * 2.5
        ...
        if state then
            ...
            activeLoops["poke"] = RunService.Heartbeat:Connect(function()
                if activeStates["poke"] and targetPlayer and isAlive(targetPlayer) and getPlayerHRP(targetPlayer) and getPlayerHRP(LocalPlayer) then
                    currentPlatform.Position = getPlayerHRP(targetPlayer).Position + Vector3.new(0,-3,0)
                    local targetPos = getPlayerHRP(targetPlayer).Position
                    local behindPos = targetPos - getPlayerHRP(targetPlayer).CFrame.LookVector * 2
...
FlingSection:Slider({
    ...
    Step = 5, Value = { Min = 10, Max = 500, Default = 50 }, IsTooltip = true, IsTextbox = true,
    Callback = function(v) PunchPower = v end
})

FlingSection:Slider({
    Title = "Fling Time", Desc = "Durasi fling dalam frames | Default: 30", Icon = "timer",
    Step = 5, Value = { Min = 5, Max = 120, Default = 30 }, IsTooltip = true, IsTextbox = true,
...
local builtinThemes = {
    ...
    "Amber", "Emerald", "Midnight", "Crimson", "Monokai Pro", "Cotton Candy", "Mellowsi", "Rainbow"
}

local customThemesFile = "WindUI/PieHub/customThemes.json"
local customThemes = {}
local selectedBuiltinTheme = ""
local selectedCustomTheme = ""
local currentAppliedTheme = "Dark"

local function saveCustomThemes()
    pcall(function()
        if writefile then writefile(customThemesFile, HttpService:JSONEncode(customThemes)) end
    end)
end

local function loadCustomThemes()
    pcall(function()
        if readfile and isfile and isfile(customThemesFile) then
            local data = HttpService:JSONDecode(readfile(customThemesFile))
            if type(data) == "table" then customThemes = data end
        end
    end)
end
...
local function getCustomThemeNames()
    local names = {}
    for name in pairs(customThemes) do table.insert(names, name) end
    table.sort(names, function(a,b) return a:lower() < b:lower() end)
    return names
end
...
local function applyThemeByName(name)
    ...
    currentAppliedTheme = name
    WindUI:Notify({ Title = "Theme", Content = "Theme '" .. name .. "' diterapkan!", Duration = 2, Icon = "palette" })
end

local function registerAndApplyCustomTheme(themeData)
    pcall(function()
        WindUI:AddTheme(themeData)
        WindUI:SetTheme(themeData.Name)
        currentAppliedTheme = themeData.Name
    end)
end

-- Section: Theme
local SectionTheme = TabThemes:Section({ Title = "Theme", Icon = "palette", Opened = true })

local themeDropdown = SectionTheme:Dropdown({
    Title = "Daftar Theme", Desc = "Pilih theme bawaan WindUI",
    Values = builtinThemes, Value = "Dark",
    Callback = function(v) selectedBuiltinTheme = v end
})

local HStackThemeBtn = SectionTheme:HStack({ AutoSpace = true })
HStackThemeBtn:Button({
    Title = "Apply Theme", Icon = "check",
    Callback = function()
        if selectedBuiltinTheme == "" then
            WindUI:Notify({ Title = "Error", Content = "Pilih theme dulu!", Duration = 2, Icon = "alert-circle" }) return
        end
        applyThemeByName(selectedBuiltinTheme)
    end
})
HStackThemeBtn:Button({
    Title = "Reset Theme", Icon = "refresh-cw",
    Callback = function()
        applyThemeByName("Dark")
        themeDropdown:Select("Dark")
        selectedBuiltinTheme = "Dark"
        WindUI:Notify({ Title = "Theme", Content = "Theme direset ke Dark.", Duration = 2, Icon = "refresh-cw" })
    end
})

SectionTheme:Divider()

-- Section: Custom Theme
local SectionCustomTheme = TabThemes:Section({ Title = "Custom Theme", Icon = "brush", Opened = true })

SectionCustomTheme:Input({
    Title = "Nama Theme", Desc = "Nama untuk custom theme",
    Placeholder = "Contoh: My Theme", Value = "",
    Callback = function(v)
        if _G.PieHubCustomThemeData then _G.PieHubCustomThemeData.Name = v end
    end
})

-- Init custom theme data
_G.PieHubCustomThemeData = {
...
local VStackColors = SectionCustomTheme:VStack({})

VStackColors:Colorpicker({
    Title = "Accent", Desc = "Warna aksen utama",
    Default = Color3.fromHex("#18181b"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Accent = c end
})

VStackColors:Colorpicker({
    Title = "Background", Desc = "Warna latar belakang",
    Default = Color3.fromHex("#101010"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Background = c end
})

VStackColors:Colorpicker({
    Title = "Outline", Desc = "Warna garis tepi",
    Default = Color3.fromHex("#FFFFFF"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Outline = c end
})

VStackColors:Colorpicker({
    Title = "Text", Desc = "Warna teks",
    Default = Color3.fromHex("#FFFFFF"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Text = c end
})

VStackColors:Colorpicker({
    Title = "Placeholder", Desc = "Warna teks placeholder",
    Default = Color3.fromHex("#7a7a7a"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Placeholder = c end
})

VStackColors:Colorpicker({
    Title = "Button", Desc = "Warna tombol",
    Default = Color3.fromHex("#52525b"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Button = c end
})

VStackColors:Colorpicker({
    Title = "Icon", Desc = "Warna ikon",
    Default = Color3.fromHex("#a1a1aa"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Icon = c end
})

VStackColors:Colorpicker({
    Title = "Hover", Desc = "Warna saat hover",
    Default = Color3.fromHex("#3f3f46"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Hover = c end
})

VStackColors:Colorpicker({
    Title = "Toggle", Desc = "Warna toggle background",
    Default = Color3.fromHex("#52525b"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Toggle = c end
})

VStackColors:Colorpicker({
    Title = "Toggle Bar", Desc = "Warna bar toggle",
    Default = Color3.fromHex("#FFFFFF"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.ToggleBar = c end
})

VStackColors:Colorpicker({
    Title = "Slider", Desc = "Warna slider background",
    Default = Color3.fromHex("#52525b"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.Slider = c end
})

VStackColors:Colorpicker({
    Title = "Slider Thumb", Desc = "Warna thumb slider",
    Default = Color3.fromHex("#FFFFFF"), Transparency = 0,
    Callback = function(c) _G.PieHubCustomThemeData.SliderThumb = c end
})
...
local function themeDataToSerializable(data)
    local t = {}
...
local function serializableToThemeData(t)
    local data = {}
    local colorKeys = {
        "Accent","Background","Outline","Text","Placeholder","Button","Icon",
        "Hover","Toggle","ToggleBar","Slider","SliderThumb",
        "WindowBackground","WindowShadow","TabBackground","TabTitle","TabIcon",
        "ElementBackground","ElementTitle","ElementDesc","ElementIcon",
    }
    for k, v in pairs(t) do
        local isColorKey = false
        for _, ck in ipairs(colorKeys) do
            if k == ck then isColorKey = true break end
        end
        if isColorKey and type(v) == "string" then
...
SectionCustomTheme:Divider()

local customThemeDropdown = SectionCustomTheme:Dropdown({
    Title = "Daftar Custom Theme", Desc = "Custom theme tersimpan",
    Values = getCustomThemeNames(), Value = "",
    Callback = function(v) selectedCustomTheme = v end
})

local HStackCustomThemeBtn = SectionCustomTheme:HStack({ AutoSpace = true })

HStackCustomThemeBtn:Button({
    Title = "Save", Icon = "save",
    Callback = function()
        local name = _G.PieHubCustomThemeData and _G.PieHubCustomThemeData.Name or ""
        if name == "" or name == "My Custom Theme" then
            WindUI:Notify({ Title = "Error", Content = "Masukkan nama theme dulu!", Duration = 2, Icon = "alert-circle" }) return
        end
        local serializable = themeDataToSerializable(_G.PieHubCustomThemeData)
        customThemes[name] = serializable
        saveCustomThemes()
        customThemeDropdown:Refresh(getCustomThemeNames())
        WindUI:Notify({ Title = "Custom Theme", Content = "Theme '" .. name .. "' disimpan!", Duration = 2, Icon = "save" })
    end
})
...
HStackCustomThemeBtn:Button({
    ...
    Callback = function()
        customThemeDropdown:Refresh(getCustomThemeNames())
        WindUI:Notify({ Title = "Custom Theme", Content = "Daftar diperbarui.", Duration = 2, Icon = "refresh-cw" })
    end
})
...
HStackCustomThemeBtn:Button({
    Title = "Delete", Icon = "trash",
    Callback = function()
        if selectedCustomTheme == "" or not customThemes[selectedCustomTheme] then
            WindUI:Notify({ Title = "Error", Content = "Pilih custom theme dulu!", Duration = 2, Icon = "alert-circle" }) return
        end
        customThemes[selectedCustomTheme] = nil
        selectedCustomTheme = ""
        saveCustomThemes()
        customThemeDropdown:Refresh(getCustomThemeNames())
        WindUI:Notify({ Title = "Custom Theme", Content = "Theme dihapus.", Duration = 2, Icon = "trash" })
    end
})

SectionCustomTheme:Divider()

local HStackCustomThemeApply = SectionCustomTheme:HStack({ AutoSpace = true })

HStackCustomThemeApply:Button({
    Title = "Apply", Icon = "check",
    Callback = function()
        if selectedCustomTheme == "" or not customThemes[selectedCustomTheme] then
            WindUI:Notify({ Title = "Error", Content = "Pilih custom theme dulu!", Duration = 2, Icon = "alert-circle" }) return
        end
        local data = serializableToThemeData(customThemes[selectedCustomTheme])
        data.Name = selectedCustomTheme
        registerAndApplyCustomTheme(data)
        WindUI:Notify({ Title = "Custom Theme", Content = "Theme '" .. selectedCustomTheme .. "' diterapkan!", Duration = 2, Icon = "check" })
    end
})

HStackCustomThemeApply:Button({
    Title = "Apply Preview", Icon = "eye",
    Callback = function()
        if not _G.PieHubCustomThemeData or (_G.PieHubCustomThemeData.Name == "" or _G.PieHubCustomThemeData.Name == "My Custom Theme") then
            WindUI:Notify({ Title = "Error", Content = "Beri nama theme dulu!", Duration = 2, Icon = "alert-circle" }) return
        end
        registerAndApplyCustomTheme(_G.PieHubCustomThemeData)
        WindUI:Notify({ Title = "Custom Theme", Content = "Preview theme diterapkan!", Duration = 2, Icon = "eye" })
    end
})
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- Reapply theme setelah respawn
    if currentAppliedTheme and currentAppliedTheme ~= "" then
        pcall(function() WindUI:SetTheme(currentAppliedTheme) end)
    end

    -- Recreate fling tools setelah respawn
    task.wait(0.5)
    if punchActive then createPunchFling() end
    if saitamaActive then createSaitamaFling() end
    if kickActive then createKickFling() end
end)
